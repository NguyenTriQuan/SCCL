...
class Appr(object):
    ...
    def __init__(self, model, nepochs=100, sbatch=256, lr=0.001, 
        ...
        self.log_name = '{}_{}_{}_{}_alpha_{}_beta_{:.5f}_ratio_{:.4f}_mul_{}_lr_{}_lr_rho_{}_batch_{}_epoch_{}'.format(
    ...
    def train_epoch(self,t,x,y):
        ...
        for i in range(0, len(r), self.sbatch):
            ...
            if self.split:
                output = F.log_softmax(self.model(t, images, sample=True),dim=1)
            else:
                output = self.model(t, images, sample=True)
    ...
    def eval(self,t,x,y):
        ...
        with torch.no_grad():
            for i in range(0, len(r), self.sbatch):
                ...
                mini_batch_size = len(targets)
                if self.split:
                    output = F.log_softmax(self.model(t, images, sample=False),dim=1)
                else:
                    output = self.model(t, images, sample=False)
                loss = F.nll_loss(output, targets, reduction='sum')
