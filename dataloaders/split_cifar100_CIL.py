...
def get(seed=0,pc_valid=0.10, tasknum = 20):
    ...
    if not os.path.isdir('../dat/binary_split_cifar100_CIL/'):
        os.makedirs('../dat/binary_split_cifar100_CIL')
        ...
        for t in range(10):
            for s in ['train','test']:
                ...
                torch.save(data[t][s]['x'], os.path.join(os.path.expanduser('../dat/binary_split_cifar100_CIL'),
                                                         'data'+str(t+1)+s+'x.bin'))
                torch.save(data[t][s]['y'], os.path.join(os.path.expanduser('../dat/binary_split_cifar100_CIL'),
    ...
    for i in range(10):
        ...
        for s in ['train','test']:
            data[i][s]={'x':[],'y':[]}
            data[i][s]['x']=torch.load(os.path.join(os.path.expanduser('../dat/binary_split_cifar100_CIL'),
                                                    'data'+str(ids[i])+s+'x.bin'))
            data[i][s]['y']=torch.load(os.path.join(os.path.expanduser('../dat/binary_split_cifar100_CIL'),
