...
class _DynamicLayer(nn.Module):
    ...
    def expand(self, add_in=None, add_out=None, ablation='full'):
        ...
        self.kbts_mask.append(mask.detach().clone().bool())
    ...
    def get_mem_params(self):
        ...
        nn.init.kaiming_uniform_(self.score, a=math.sqrt(5))
        mask = GetSubnet.apply(self.score.abs(), self.sparsity)
        self.jr_mask = mask.detach().clone().bool()

    def forward(self, x, t, kbts=False, jr=False):    
        weight, bias = self.get_params(t, kbts, jr)
    ...
    def get_params(self, t, kbts=False, jr=False):
        ...
        if kbts or jr:
            ...
            if self.training:
                ...
                if kbts:
                    self.kbts_mask[t] = mask.detach().clone().bool()
                else:
                    self.jr_mask = mask.detach().clone().bool()
            else:
                if kbts:
                    weight = weight * self.kbts_mask[t] / self.sparsity
                else:
                    weight = weight * self.jr_mask / self.sparsity
