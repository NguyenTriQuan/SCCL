...
class _DynamicLayer(nn.Module):
    ...
    def get_parameters(self, t):
        ...
        for i in range(1, t):
            self.bwt_weight[i] *= self.bwt_scale[t][i].view(view)
            self.fwt_weight[i] *= self.fwt_scale[t][i].view(view)
            self.weight[i] *= self.fwt_scale[t][i].view(view)
